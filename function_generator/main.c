 ...
 * Input from PB4 (ADC2) to Calibrate Output Frequency, ADC Value 0 Means -15, ADC Value 255 Means +15
...
int main(void) {
	...
	while(1) {
		...
		if ( value_adc_channel_1_high_buffer != value_adc_channel_1_high ) {
			...
			if ( value_adc_channel_1_high >= 240 ) {
				count_per_2pi_buffer = 35; // C6 1046.50 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 224 ) {
				count_per_2pi_buffer = 37; // B5 987.77 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 208 ) {
				count_per_2pi_buffer = 39; // A#5 932.33 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 192 ) {
				count_per_2pi_buffer = 41; // A5 880.00 Hz
				osccal_tuning = 0;
			} else if ( value_adc_channel_1_high >= 176 ) {
				count_per_2pi_buffer = 44; // G#5 830.61 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 160 ) {
				count_per_2pi_buffer = 47; // G5 783.99 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 144 ) {
				count_per_2pi_buffer = 50; // F#5 739.99 Hz
				osccal_tuning = 2;
			} else if ( value_adc_channel_1_high >= 128 ) {
				count_per_2pi_buffer = 53; // F 698.46 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 112 ) {
				count_per_2pi_buffer = 56; // E 659.26
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 96 ) {
				count_per_2pi_buffer = 59; // D#5 622.25 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 80 ) {
				count_per_2pi_buffer = 63; // D5 587.33 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 64 ) {
				count_per_2pi_buffer = 67; // C#5 554.37 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 32 ) {
				count_per_2pi_buffer = 70; // C5 523.25 Hz
				osccal_tuning = 1;
			} else if ( value_adc_channel_1_high >= 16 ) {
		...
		if ( value_adc_channel_2_high_buffer != value_adc_channel_2_high ) {
			...
			// Convert (0)-(255) to (-127)-(127) by EOR with 0x80, Arithmetic Logical Shift Right for Range (-15)-(15)
