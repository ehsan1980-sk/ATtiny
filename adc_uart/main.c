...
int main(void) {
	...
	// Set ADC, Vcc as Reference, No ADLAR
	ADMUX = 0;
	...
	while(1) {
		...
		value_adc = ADCH<<8|ADCL; // Read Low Bits First
		value_adc |= ADCH<<8; // ADC[9:0] Will Be Updated After High Bits Are Read.

		software_uart_tx_9600( (uint8_t)value_adc, software_uart_tx_pin );
		software_uart_tx_9600( (uint8_t)value_adc>>8, software_uart_tx_pin );
		_delay_ms( 500 );
	}
	return 0;
}
